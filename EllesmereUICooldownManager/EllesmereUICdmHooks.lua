...
-------------------------------------------------------------------------------
--  Party Mode: spinning Cooldown Manager. Like the action bar spin, icons
--  orbit by re-anchoring, so they stay upright. The rest position is read
--  from the live fd._cdmAnchor each tick, so rebuilds never drift, and the
--  orbit SetPoint targets the same container so the per-icon guard leaves it
--  alone. In combat the icons snap to rest and hold: Blizzard re-lays viewer
--  frames in combat and the guard would scatter a frozen mid-orbit pose.
-------------------------------------------------------------------------------
do
local spinDriver, spinAngle, spinHeld = nil, 0, false
...
local function SpinOn()
    return EllesmereUIDB and EllesmereUIDB.partyMode
        and EllesmereUIDB.partyModeSpinCDM and true or false
end

-- Orbit pivot: a bar anchored to another CDM bar (Unlock Mode anchor, or the
-- bar's own Anchor To setting) orbits around the ROOT bar of its chain, so
-- small satellite bars (trinkets, CC) swing round with the bar they hang off
-- instead of tumbling in place around their own tiny centre. Unanchored bars
-- pivot on themselves, exactly as before.
local function SpinRootBar(barKey)
    local ua = EllesmereUIDB and EllesmereUIDB.unlockAnchors
    local k, hops = barKey, 0
    while hops < 8 do
        local nextKey
        local ai = ua and ua["CDM_" .. k]
        local t = ai and ai.target
        if type(t) == "string" and t:sub(1, 4) == "CDM_" and cdmBarFrames[t:sub(5)] then
            nextKey = t:sub(5)
        else
            local bd = ns.barDataByKey and ns.barDataByKey[k]
            local at = bd and bd.anchorTo
            if type(at) == "string" and cdmBarFrames[at] then nextKey = at end
        end
        if not nextKey or nextKey == barKey then break end
        k, hops = nextKey, hops + 1
    end
    return cdmBarFrames[k]
end

local function SpinTick(c, s)
    for barKey, icons in pairs(cdmBarIcons) do
        local own = icons and icons[1] and cdmBarFrames[barKey]
        if own and own:IsVisible() then
            -- Screen-space shift from the pivot's centre to this bar's centre.
            local root = SpinRootBar(barKey) or own
            if not root:IsVisible() then root = own end
            local ox, oy = 0, 0
            if root ~= own then
                local ocx, ocy = own:GetCenter()
                local rcx, rcy = root:GetCenter()
                if ocx and rcx then
                    local os, rs = own:GetEffectiveScale(), root:GetEffectiveScale()
                    ox, oy = ocx * os - rcx * rs, ocy * os - rcy * rs
                end
            end
            for i = 1, #icons do
                local icon = icons[i]
                local fd = icon and hookFrameData[icon]
                local a = fd and fd._cdmAnchor
                local bar = a and a[2]
                if bar and a[1] == "TOPLEFT" and a[3] == "TOPLEFT"
                   and icon:IsShown() and bar:IsVisible() then
                    local is, bs = icon:GetEffectiveScale(), bar:GetEffectiveScale()
                    if is and is > 0 and bs then
                        -- Bar half-size converted into the icon's units.
                        local k = bs / is
                        local hw = (icon:GetWidth() or 0) * 0.5
                        local hh = (icon:GetHeight() or 0) * 0.5
                        local bw = (bar:GetWidth() or 0) * 0.5 * k
                        local bh = (bar:GetHeight() or 0) * 0.5 * k
                        -- Resting centre offset from the PIVOT, in screen px.
                        local px = (a[4] + hw - bw) * is + ox
                        local py = (a[5] - hh + bh) * is + oy
                        -- Rotate, then back to icon units relative to this bar.
                        local rx = (px * c - py * s - ox) / is
                        local ry = (px * s + py * c - oy) / is
                        icon:ClearAllPoints()
...
function ns.PartySpinCDM_Refresh()
    if not SpinOn() then
        if spinDriver and spinDriver:IsShown() then
            spinDriver:Hide()
            spinAngle, spinHeld = 0, false
            ReapplyPositions()
        end
        return
    end
    if not spinDriver then
        ...
        spinDriver:SetScript("OnUpdate", function(_, elapsed)
            ...
            if not SpinOn() then
                ns.PartySpinCDM_Refresh()
                return
            end
            if InCombatLockdown() then
                if not spinHeld then
                    spinHeld = true
                    ReapplyPositions()
                end
                return
            ...
            SpinTick(math.cos(spinAngle), math.sin(spinAngle))
        end)
    end
    spinDriver:Show()
end
...
EllesmereUI.PartySpinCDM_Refresh = ns.PartySpinCDM_Refresh

-- Party Mode starts from the options page, a keybind, a random timer, or
-- Bloodlust; hooking its two public entry points catches all of them. The
-- login start fires on PLAYER_LOGIN, after this file has loaded.
if EllesmereUI_StartPartyMode then
    hooksecurefunc("EllesmereUI_StartPartyMode", function() ns.PartySpinCDM_Refresh() end)
end
if EllesmereUI_StopPartyMode then
    hooksecurefunc("EllesmereUI_StopPartyMode", function() ns.PartySpinCDM_Refresh() end)
end
end
